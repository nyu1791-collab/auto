...
| --------- | ------------ | ------------ | -------------- | -------- | ----------- | ----------- |
| 弱(Light) | 16 (267ms)   | 2            | 12             | 10       | 70 px       | 52 px       |
| 強(Heavy) | 32 (533ms)   | 3            | 30             | 30       | 90 px       | 60 px       |

> 発生(windup)は「見てから回避するための猶予」そのもの。易化のため弱 9→16(150→267ms)・
> 強 27→32(450→533ms)へ延ばし、攻撃が速すぎて反応できない問題を解消した
> (人型の攻撃モーション・テレグラフ・アシストリングもその分長く表示される)。
...
| 項目          | 値                                                                           |
| ------------- | ---------------------------------------------------------------------------- |
| 無敵(i-frame) | 開始から 13 tick(≈217ms。`JUST.window`(11)より大きく取り報酬窓を使い切れるようにする) |
| 総持続        | 18 tick                                                                      |
| クールダウン  | 持続終了後 6 tick                                                            |
| スタミナ消費  | 35                                                                           |
| 移動          | 回避方向へ 2 px/tick × i-frame 中(最大 26px。攻撃の間合いより十分小さく保つ) |
...
- 防御側が回避を**開始した tick** が、攻撃の active 開始の **11 tick 前以内**(≈183ms)だった場合
  → **ジャスト回避**:攻撃側を 24 tick(≈400ms)硬直 + 防御側にスタミナ 35 全額還元。
- それより早く出した回避(12 tick 以上前)→ ダメージは無効だが報酬なし(早すぎる読み回避)。

> 当初は 7 tick(≈120ms)だったが、反応がやや忙しかったため 7→9→11 tick(≈183ms)へ段階的に
> 緩和(易化)した。loose ボットの `reactionDelay`(15)未満に保つ限り、ヘッドレスシミュレーションの
> 構造的な基準結果(勝敗・引き分け)は変わらない(攻撃が遅くなった分、決着までの平均 tick は伸びる)。
...
| ------ | ------------- | ------------------ | --------------- | ------------------------------------------ |
| easy   | 12            | 0.6                | false           | 反応が遅くサボりがち。切り返さない(早読みのみ) |
